...
CMD2Model::CMD2Model():
m_fScale(5), m_uiTexID(0), anime(false), animeCurrentFrame(-1), animeType(LOOP)
{}
...
bool CMD2Model::LoadModel( std::string szFilename )
{
    ...
    // vecteur pour ls normales améliorées
	normals = new vec3_t[m_kHeader.num_vertices];
...
void CMD2Model::LoadTexture( std::string szFilename )
{
    ...
    int width, height;
    unsigned char* image = load_PPM(szFilename.c_str(), &width, &height);
...
void CMD2Model::RenderFrame( int iFrame )
{
    ...
    // calcul des normales optimisées
    /*for( int i = 0; i < m_kHeader.num_vertices; i++ ) // on parcourt tous les vertex
	{	
		std::vector< std::pair<vec3_t, float> > verticesTrianglesNormals;
			
		for(int j=0; j<m_kHeader.num_tris; ++j)
		{
			//if(egalite(m_pTriangles[i].vertex[j], ) || egalite() || egalite())
		}
...
{    
	...
	float frameTime = (1.0/(float)fps) * 1000;
