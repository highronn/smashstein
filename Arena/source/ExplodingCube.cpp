...
ExplodingCube::ExplodingCube(void){
...
void ExplodingCube::drawProjectiles(RTPoint::point3 camera,unsigned int idTexProjectile,unsigned int idTexSystemParticles)
{
...
std::pair<unsigned int,unsigned int> ExplodingCube::loadTextureProjectile(void)
{
	...
	int longueurH;
  	...
  	unsigned char *tex_cube = NULL;
  	tex_cube = load_PPM_RGBA("projectiles/cube.ppm", &largeurW, &longueurH, RTColor::color(0.0,0.0,0.0), NORMAL);
  	
	unsigned int idCube;
	InitGLRGBA(tex_cube, largeurW, longueurH, idCube);

	std::pair<unsigned int,unsigned int> resPair = std::make_pair(idCube, 0);
	return resPair;
}

void ExplodingCube::updateProjectiles(void)
{}
