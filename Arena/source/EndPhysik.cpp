...
EndPhysik::EndPhysik(float speedo,int lifeTimeMaximum,float size,int nbMaxParticless){
...
void EndPhysik::drawSystemParticles(RTPoint::point3 camera,unsigned int idTexSystemParticles){
	...
	for(int i=0;i<(int)particles.size();++i){
		float angle = rand()*360.0f/RAND_MAX;
		glPushMatrix();
...
void EndPhysik::updateSystemParticles(void){
...
void EndPhysik::generateParticles(void){
