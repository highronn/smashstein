...
class IOMesh : public Mesh, public kn::smashstein::Obstacle
{		
	public:
		std::string name_;
		double x_, y_, z_;
	...
	private :
		Wrap boundingBox;
	public :
		inline const Wrap & getBoundingBox() const {return boundingBox;}
		inline Wrap & getBoundingBox() {return boundingBox;}
	
	// fonctions pour détecter la collision
	public:
		std::pair<collisionFace, float> hitTestObject(const IOMesh &);
		std::pair<collisionFace, float> hitTestObject(const Wrap &);
